...
#define SYSTEC_MODULE_VERSION   "1.0.1"
...
static int systec_high_performance(struct systec_can_shared * shared_res, bool* high_performance, u8* reg_value);
...
static netdev_tx_t systec_can_start_xmit(struct sk_buff *skb, struct net_device *netdev)
{
        ...
        if (cf->can_id & CAN_EFF_FLAG) {
                ...
                if (likely((chan->flags & SYSTEC_CAN_CHANNEL_FLAGS_CLEAR_TX) == 0)) {
                        ...
                        put_unaligned_be16(USBCAN_DATACANID_ENDOFRESET,
                                &msg->msg.can_std_msg.can_id);
                }
        }
...
static ssize_t systec_can_sysfs_show_high_performance(struct device *dev,
                struct device_attribute *attr, char *buf)
{
        struct usb_interface *intf = to_usb_interface(dev);
        struct systec_can_shared *shared_res = usb_get_intfdata(intf);
...
static ssize_t systec_can_sysfs_store_high_performance(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
{
        struct usb_interface *intf = to_usb_interface(dev);
        struct systec_can_shared *shared_res = usb_get_intfdata(intf);
...
static DEVICE_ATTR(high_performance, S_IRUGO | S_IWUSR,
        systec_can_sysfs_show_high_performance, systec_can_sysfs_store_high_performance);
                            ...
                            const struct usb_device_id *id)
{
        ...
        shared_res->tx_cmd_buffer = devm_kzalloc(&intf->dev, sizeof(struct systec_cmd_msg), GFP_KERNEL);
        if (!shared_res->tx_cmd_buffer) {
                dev_err(&intf->dev, "Couldn't alloc Tx cmd buffer\n");
                return (ret_stat);
        }

        shared_res->rx_cmd_buffer = devm_kzalloc(&intf->dev, sizeof(struct systec_cmd_msg), GFP_KERNEL);
